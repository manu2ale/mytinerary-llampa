...
export default function CardCity({ imgUrl, city, country, id }) {
  ...
  return (
    <Anchor to={'/city/' + id} className='flex group w-[174px] h-[120px] relative hover:cursor-pointer hover:outline outline-offset-0 outline-teal-500 rounded-2xl overflow-hidden text-white font-semibold
      ...
      <p className='absolute top-0 border-t-2 h-8 w-full text-center sm:group-hover:translate-y-20 duration-500 ease-in-out align-middle text-xl bg-gray-500/50 backdrop-blur-sm rounded-t-2xl  sm:text-2xl sm:bg-teal-500/50'>{city}</p>
      <p className='absolute top-8 border-b-2 h-8 w-full text-center sm:group-hover:translate-y-20 duration-500 ease-in-out align-middle backdrop-blur-sm rounded-b-2xl bg-gray-500/50 sm:bg-teal-500/50'>
